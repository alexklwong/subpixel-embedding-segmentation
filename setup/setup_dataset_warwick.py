...
def store_scan_paths():
    ...
    if not os.path.exists(WARWICK_RAW_DATA_DIRPATH):
        raise ValueError("Path to raw data {} does not exist.".format(WARWICK_RAW_DATA_DIRPATH))

    for filename in os.listdir(WARWICK_RAW_DATA_DIRPATH):
        _, extension = os.path.splitext(filename)
        raw_save_path = os.path.join(WARWICK_RAW_DATA_DIRPATH, filename)
        raw_ground_truth_save_path = raw_save_path.replace('.bmp', '_anno.bmp')
        numpy_filename = filename.replace(extension, '_scan.npy')
        numpy_save_path = os.path.join(WARWICK_NUMPY_DATA_DIRPATH, numpy_filename)
        numpy_ground_truth_save_path = numpy_save_path.replace('_scan', '_anno')
        ...
        if 'train' in filename:
            # Get paths for scan (raw and numpy)
            train_traintest_scan_input_paths.append(raw_save_path)
            train_traintest_scan_output_paths.append(numpy_save_path)
            # Get paths for ground truth (raw and numpy)
            train_traintest_ground_truth_input_paths.append(raw_ground_truth_save_path)
            train_traintest_ground_truth_output_paths.append(numpy_ground_truth_save_path)
        elif 'test' in filename:
            # Get paths for scan (raw and numpy)
            test_traintest_scan_input_paths.append(raw_save_path)
            test_traintest_scan_output_paths.append(numpy_save_path)
            # Get paths for ground truth (raw and numpy)
            test_traintest_ground_truth_input_paths.append(raw_ground_truth_save_path)
            test_traintest_ground_truth_output_paths.append(numpy_ground_truth_save_path)
...
def setup_dataset(paths_only=False):
    ...
    train_traintest_ground_truth_output_paths.sort()
    test_traintest_scan_output_paths.sort()
    test_traintest_ground_truth_output_paths.sort()

    data_utils.write_paths(TRAIN_TRAINTEST_SCAN_OUTPUT_FILEPATH, train_traintest_scan_output_paths)
    data_utils.write_paths(TRAIN_TRAINTEST_GROUND_TRUTH_OUTPUT_FILEPATH, train_traintest_ground_truth_output_paths)
    data_utils.write_paths(TEST_TRAINTEST_SCAN_OUTPUT_FILEPATH, test_traintest_scan_output_paths)
    data_utils.write_paths(TEST_TRAINTEST_GROUND_TRUTH_OUTPUT_FILEPATH, test_traintest_ground_truth_output_paths)
