...
def get_data(num=[0, 304],
             dataset_path='data/atlas/atlas_standard'):
    # path_csv = 'data/atlas/atlas_/atlas_metadata.csv'
    ...
    standard_data = glob.glob(os.path.join(dataset_path, "*/*"))
    print(len(standard_data))

    for file_path in tqdm(standard_data):
            ...
            for file in files:
                ...
                if deface_file == 't1w':
                    tem_deface = load_nii(os.path.join(
                        file_path, file)).transpose((2, 1, 0)) / 100

                elif seg_file == 'LesionSmooth':
                    tem = load_nii(os.path.join(file_path, file)).transpose(
                        (2, 1, 0)).astype(np.int64)
                    tem[tem > 0] = 1
                    tem_seg.append(tem)

        deface.append(tem_deface)
        data_order.append(file_path)
    ...
    # seg = np.array(seg)
    # deface /= 100
    print('number of images found: {}, number of segmentation found: {}'.format(
        len(deface), len(seg)))
    return deface, seg, data_order
...
def train_data_generator(save_path, dataset_path='data/atlas/atlas_standard'):
    h5_path = 'ATLAS.h5'
    if os.path.exists(h5_path) == False:
        deface, seg, data_order = get_data([0, 228], dataset_path=dataset_path)
        deface = np.array(deface)
        deface_slice_train, seg_slice_train = to_slice(
            deface[:], seg[:], 'all')

        print('generating h5 file for ATLAS dataset')
        file_train = h5py.File(os.path.join(save_path, 'train.h5'), 'w')
        file_train.create_dataset('data', data=deface_slice_train)
        file_train.create_dataset('label', data=seg_slice_train)
        file_train.close()

        with open(os.path.join(save_path, 'data_order.txt'), 'w') as data_order_file:
            for file_name in data_order:
                data_order_file.write('{}\n'.format(file_name))



if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset-path', default='data/atlas/atlas_standard', type=str,
                        help='path of ATLAS_R1.1')
    parser.add_argument('--save_path', type=str, help='path to save the output h5 file')
    args = parser.parse_args()
    train_data_generator(save_path=args.save_path, dataset_path=args.dataset_path)
