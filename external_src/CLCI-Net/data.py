...
def train_data_generator(patient_indexes, h5_file_path, batch_size):
    ...
    while True:
        ...
        for b in range(batch_size):
            ...
            current_img = imgs[slice_indexes[i]][5:229, 11:187] # changed this so that the image size is 224*176
            current_label = labels[slice_indexes[i]][5:229, 11:187]
...
def val_data_generator(patient_indexes, h5_file_path, batch_size=1):
    ...
    while True:
        ...
        for b in range(batch_size):
            current_img = imgs[slice_indexes[i]][5:229, 11:187] # changed this so that the image size is 224*176
            current_label = labels[slice_indexes[i]][5:229, 11:187] # changed this so that the image size is 224*176
            batch_img.append(current_img)
            batch_label.append(current_label)
            i = (i + 1) % num_of_slices
        yield np.expand_dims(np.array(batch_img), 3), np.expand_dims(np.array(batch_label), 3)
...
def read_data_split(data_split_path, data_order_file):
    ...
    data_to_index_dict = get_data_to_index_dict(data_order_file)
