...
PREDICTION_PATH = "trained_model/DUnet/trial1/outputs"
VALIDATION_PATH = "../stroke-lesion-segmentation/validation/atlas/trainval/atlas_trainval_val_annotations.txt"
...
n_classes = 2
size = (192, 192)
log_path = "trained_model/DUnet/trial1/validation/val_result.txt"
small_lesion_only = False
visual_paths = ["trained_model/DUnet/trial1/visual/all"]
...
if __name__ == "__main__":
    ...
    sl_dice_scores = np.zeros(n_samples)
    sl_ious = np.zeros(n_samples)
    sl_precisions = np.zeros(n_samples)
    sl_recalls = np.zeros(n_samples)
    sl_dice_from_ext = np.zeros(n_samples)
    ...
    sl_per_class_dices = np.zeros((n_samples, n_classes))
    sl_per_class_ious = np.zeros((n_samples, n_classes))
    sl_per_class_precisions = np.zeros((n_samples, n_classes))
    sl_per_class_recalls = np.zeros((n_samples, n_classes))

    gt_paths = []
    with open(VALIDATION_PATH, 'r') as val_anno:
        gt_paths = val_anno.readlines()
    gt_paths = ['../stroke-lesion-segmentation/{}'.format(gt_path.rstrip()) for gt_path in gt_paths]
    ...
    for idx, path in enumerate(output_paths):
        ...
        # # This is for D-Unet
        output_np = np.squeeze(output_np, axis=3)
        output_sig = output_np
        ...
        gt_resized[gt_resized >= 0.5] = 1
        gt_resized[gt_resized < 0.5] = 0
        print(gt_resized.shape)

        # results for small lesion samples
        ...
        sl_class_histogram = compute_prediction_hist(np.int64(sl_gt).flatten(), np.int64(sl_out).flatten(), n_classes)
        ...
        if len(visual_paths) > 0:
            site = patient[:5]
            input_path = "../stroke-lesion-segmentation/data/atlas_lesion_segmentation/{}/{}/{}_t1w_stx.npy".format(site, patient, patient)
            image = np.load(input_path)
            image = image.transpose((2,1,0))
            image_resized = []
            for i in range(gt_np.shape[0]):
                image_resize_single = Image.fromarray(image[i, ...]).crop((10, 40, 190, 220))
                image_resize_single = image_resize_single.resize(size, Image.ANTIALIAS)
                image_resize_single = np.asarray(image_resize_single)

                image_resized.append(image_resize_single)
            ...
            for i in range(output_np.shape[0]):
                ...
                save_prediction_img(
                    ...
                    chunk_idx=i,
                    output_segmentation=output_np[i],
